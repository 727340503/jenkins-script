...
echo "scp -P $server_port $check_tomcat_pid_file $server_user@$server_host:$remote_war_path"
scp -p $server_port $check_tomcat_pid_file $server_user@$server_host:$remote_war_path
...
echo "sh $remote_war_path/check_tomcat_pid.sh"
sh $remote_war_path/check_tomcat_pid.sh

echo "delete check tomcat pid sh file"
rm -rf /remote_war_path/check_tomcat_pid.sh
